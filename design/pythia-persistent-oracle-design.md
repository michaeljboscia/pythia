...
47. **In-memory async-mutex for GeminiRuntime:** MCP SDK dispatches tool calls concurrently without awaiting. All async read-modify-write sequences on the `GeminiRuntime` singleton (daemon pool, active tool executions) are protected by `async-mutex`. File-system concurrency (`writeStateWithRetry`) and in-memory concurrency (mutex) are complementary — both required. New dependency: `async-mutex`. Research: both Gemini and Codex flagged as critical gap.
48. **PPID watchdog + startup orphan sweep:** Claude Code sends `SIGKILL` on session end — `process.on('exit')` never fires. `GeminiRuntime` runs a `setInterval` PPID watchdog that detects parent death and executes tree-kill on all active daemons. On MCP server startup, sweeps for orphaned PTY processes from previous crashes using PID files. Research: both Gemini and Codex flagged as critical.
49. **Exact token counting as primary pressure signal:** `countTokens` API / response `usage_metadata` is the primary pressure signal, replacing chars/4 heuristic. `token_count_method` in `state.json` tracks active mode (`"exact"` or `"estimate"`). Falls back to char heuristic if `countTokens` call fails. Rationale: network is already required for daemon operation — no additional availability cost. Research: Codex identified chars/4 as "technically outdated."
50. **OpenTelemetry tracing:** Real OTel instrumentation via `@opentelemetry/api`, `@opentelemetry/sdk-node`, `@opentelemetry/exporter-trace-otlp-http`. Every MCP tool call is a span. `trace_id`, `span_id`, `parent_span_id` written to `InteractionEntry`. OTLP exporter env-gated via `OTEL_EXPORTER_OTLP_ENDPOINT` — unset = spans silently dropped. When Grafana Tempo is stood up, traces flow instantly with zero code changes.
51. **Checkpoint temperature 0 + citation enforcement:** Checkpoint generation prompt hardcodes `temperature: 0` to prevent generational drift (research proves even slight temperature causes "confidently wrong" hallucinations within 3-5 generations). Prompt mandates source citations: "every claim MUST cite the source document or interaction ID it originated from. If you cannot cite it, do not include it." Research: both Gemini and Codex flagged.
